"""CLI entrypoint for jira_work_hours.

Refactored from the original root-level main.py so the project can be
installed and invoked via the console script `get-work-hours`.
"""
...
from .login_helper import ensure_credentials
            ...
            if date_logged_hours > 0:
                ...
                if isinstance(pdata, dict):
                    ...
                    if pdata.get("is_productive_activity"):
                        ...
                        productive_total_logged += pdata["logged_hours"]
                else:
...
def get_range_productivity(start_date, end_date, jira, jira_username, period_label, exclude_weekends=EXCLUDE_WEEKENDS_DEFAULT):
    ...
    try:
        ...
        for item in range_productivity:
            prod_str = _format_percent(item["productivity_score"]) if item["is_productive_activity"] else "N/A"
            rows.append([item["issue_key"], item.get("activity_type") or "-", f"{item['estimated_hours']:.2f}", f"{item['logged_hours']:.2f}", prod_str, ""])
...
def main():  # console script entry
    ...
    # Main menu loop - recursive until user chooses to exit
    while True:
        print("\n" + "="*50)
        ...
        if choice == "1":
            ...
            print("\n--- Edit Login Details ---")
            print("Updating your Jira credentials...")
            ensure_credentials(force_login=True)
            # Re-establish connection with new credentials
            new_jira, new_jira_username = connect_to_jira()
            if new_jira:
                jira = new_jira
                jira_username = new_jira_username
                print("Login details updated successfully!")
            else:
                print("Failed to connect with new credentials. Keeping existing connection.")
        elif choice == "9":
            print("Thank you for using Jira Productivity & Work Hours Tracker!")
            print("Goodbye!")
            break
        else:
            print("Invalid choice. Please enter a number between 1-9.")
        
        # Ask if user wants to continue after each operation (except exit)
        if choice != "9":
