...
- If missing or `force_login` is True, prompts user for JIRA_USERNAME and JIRA_PAT (visible input by design).
- Saves values back to OS keyring (best-effort) and sets them into `os.environ` for the running session.

This keeps secret handling centralized. The package uses a fixed company Jira URL.
"""
...
def ensure_credentials(force_login=False):
