...
from login_helper import ensure_credentials  # centralized credential prompting/storage
...
def connect_to_jira():
    # Reload global variables after ensure_credentials may have set environment
    global JIRA_URL, JIRA_USERNAME, JIRA_PAT
    JIRA_URL = os.getenv("JIRA_URL")
    JIRA_USERNAME = os.getenv("JIRA_USERNAME") 
    JIRA_PAT = os.getenv("JIRA_PAT")
...
def calculate_productivity_score(estimated_hours, logged_hours):
    ...
    ((total estimated hours - total logged hours) / total estimated hours) * 100
    
    This gives the percentage of time saved compared to the original estimate.
    - Higher score = completed faster than estimated (more efficient)
    - Lower score = took longer than estimated (less efficient)
    - Negative score = exceeded the estimate
    """
...
def get_jira_details(date_str, jira=None, jira_username=None):
    """Fetch issues created and work logged for a specific date using an existing Jira session.
    If jira/jira_username not provided, it will attempt a fresh connection (legacy behaviour)."""
    ...
    # Backward compatibility: allow standalone call
    if jira is None or jira_username is None:
            ...
            if date_logged_hours > 0:
                ...
                if isinstance(pdata, dict):
                    ...
                    if pdata.get("is_productive_activity"):
                        ...
                        productive_total_logged += pdata["logged_hours"]  # Use total logged hours, not just daily hours
                else:
            ...
            if range_hours > 0:
                ...
                if isinstance(pdata, dict):
                    ...
                    if pdata.get("is_productive_activity"):
                        ...
                        prod_log += pdata["logged_hours"]  # Use total logged hours, not just period hours
                else:
